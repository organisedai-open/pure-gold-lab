  ...
  persist(
    (set, get) => ({
      ...
      addItem: async (item) => {
        ...
        try {
          if (!cartId) {
            const res = await createShopifyCart({ variantId: item.variantId, quantity: item.quantity });
            if (res) {
              set({
                cartId: res.cartId,
            ...
            const res = await addLineToShopifyCart(cartId, { variantId: item.variantId, quantity: item.quantity });
            if (res.success) {
